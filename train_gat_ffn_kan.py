...
def set_logger(log_path: str):
    logger = logging.getLogger("train_graph_attention_ffn_kan")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("Start training GraphAttentionKAN with KAN-FFN backbone")
    ...
    if opt.resume:
        ...
        best_test_macro_f1 = ckpt.get("best_test_macro_f1", -1.0)
    ...
    for epoch in range(start_epoch, opt.epochs + 1):
        ...
        if val_metrics["acc"] > best_val_acc:
            ...
            save_checkpoint(state, best_acc_path)
            logger.info(f"✅ New best accuracy on val: {best_val_acc*100:.2f}%")

        # best by TEST macro_f1 (warning: this is test-set model selection)
        if test_metrics["macro_f1"] > best_test_macro_f1:
            ...
            best_test_path = os.path.join(opt.save_folder, f"{opt.model_name}_best_test_macro_f1.pth")
            state = {
                **common_state,
                "best_val_macro_f1": best_val_macro_f1,
                "best_val_acc": best_val_acc,
            ...
            np.save(os.path.join(opt.save_folder, "best_test_confusion_matrix.npy"), test_cm)
            with open(os.path.join(opt.save_folder, "best_test_classification_report.txt"), "w", encoding="utf-8") as f:
            ...
            logger.info(
                f"🧪 New best TEST macro-F1: {best_test_macro_f1:.4f} | saved to {best_test_path}"
            )
    ...
    logger.info(f"Best test macro-F1: {best_test_macro_f1:.4f}")
