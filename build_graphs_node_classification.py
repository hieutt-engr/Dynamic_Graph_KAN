...
def build_graph_for_window(
    ...
    # --------------------------------------------------------
    # Node labels for node classification / multi-task learning
    # --------------------------------------------------------
    node_y = torch.tensor(
        msg_window["multiclass_label_index"].to_numpy(dtype=np.int64),
        dtype=torch.long
    )

    node_mask = torch.ones(len(msg_window), dtype=torch.bool)

    node_is_attack = torch.tensor(
        (msg_window["multiclass_label"].astype(str).to_numpy() != 0).astype(np.int64),
        dtype=torch.long
    )
    ...
    graph = {
        ...
        "meta": {
            ...
            # optional node-level debug info
            "node_attack_count": int((msg_window["multiclass_label"].astype(str) != 0).sum()),
            "node_label_hist": msg_window["multiclass_label"].astype(str).value_counts().to_dict(),
        },
    }
